{ config, ... }:
let
  eDP-1 = {
    mtworx = {
      fingerprint = "";
      mode = "1920x1200";
    };
    vno1-gdrx = {
      fingerprint = "";
      mode = "0x0";
    };
  };

  vno2-DP-3 = {
    fingerprint = "";
    mode = "2560x1440";
  };

  work-DP-3 = {
    fingerprint = "";
    mode = "3840x2160";
  };

  home-DP-4-1-1-vertical = {
    fingerprint = "";
    mode = "2560x1440";
  };

  home-DP-4-8-horizontal = {
    fingerprint = "";
    mode = "2560x1600";
  };
in
{
  services.autorandr = {
    enable = true;
    matchEdid = true;

    profiles = rec {
      default = {
        fingerprint = {
          eDP-1 = eDP-1.${config.networking.hostName}.fingerprint;
        };
        config = {
          eDP-1 = {
            inherit (eDP-1.${config.networking.hostName}) mode;
            enable = true;
            primary = true;
            crtc = 0;
            position = "0x0";
          };
        };
      };

      work-lidclosed = {
        fingerprint = {
          work-DP-3 = work-DP-3.fingerprint;
        };
        config = {
          work-DP-3 = {
            inherit (work-DP-3) mode;
            enable = true;
            primary = true;
            crtc = 0;
            position = "1920x0";
          };
        };
      };

      work-vno2 = {
        fingerprint = {
          eDP-1 = eDP-1.${config.networking.hostName}.fingerprint;
          vno2-DP-3 = vno2-DP-3.fingerprint;
        };
        config = {
          vno2-DP-3 = {
            inherit (vno2-DP-3) mode;
            enable = true;
            primary = true;
            crtc = 0;
          };
          eDP-1 = {
            inherit (eDP-1.${config.networking.hostName}) mode;
            enable = true;
            crtc = 1;
            position = "320x1440";
          };
        };
      };

      work-lidopen = {
        fingerprint = {
          eDP-1 = eDP-1.${config.networking.hostName}.fingerprint;
          work-DP-3 = work-DP-3.fingerprint;
        };
        config = {
          work-DP-3 = {
            inherit (work-DP-3) mode;
            enable = true;
            primary = true;
            crtc = 0;
            position = "1920x0";
          };
          eDP-1 = {
            inherit (eDP-1.${config.networking.hostName}) mode;
            enable = true;
            crtc = 1;
            position = "0x480";
          };
        };
      };

      home-lidclosed = {
        fingerprint = {
          home-DP-4-1-1-vertical = home-DP-4-1-1-vertical.fingerprint;
          home-DP-4-8-horizontal = home-DP-4-8-horizontal.fingerprint;
        };
        config = {
          home-DP-4-8-horizontal = {
            inherit (home-DP-4-8-horizontal) mode;
            enable = true;
            crtc = 0;
            position = "1440x413";
          };
          home-DP-4-1-1-vertical = {
            inherit (home-DP-4-1-1-vertical) mode;
            enable = true;
            crtc = 1;
            rotate = "right";
          };
        };
      };

      home-lidopen = {
        fingerprint = {
          home-DP-4-1-1-vertical = home-DP-4-1-1-vertical.fingerprint;
          home-DP-4-8-horizontal = home-DP-4-8-horizontal.fingerprint;
          eDP-1 = eDP-1.${config.networking.hostName}.fingerprint;
        };
        inherit (home-lidclosed) config;
      };
    };
  };
}
