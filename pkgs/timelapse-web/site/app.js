(() => {
  'use strict';
  const FPS = 24, SPEED_FPS = [2, 4, 6, 12, 24, 36, 48, 72, 96, 144, 288, 576, 864, 1152, 1728], NATIVE_SPEED_FPS = new Set([24, 144, 864]), DEFAULT_SPEED_FPS = 144, DEFAULT_SPEED_INDEX = SPEED_FPS.indexOf(DEFAULT_SPEED_FPS), FRAME_INTERVAL_SECONDS = 300, THUMBNAIL_INTERVAL_MS = 6 * 60 * 60 * 1000, BUFFER_SECONDS = 15, SEEK_EPSILON = .01, TIME_ZONE = 'Europe/Vilnius';
  ...
  function renderPane(camera, cameras) {
    const ptzChrome = cameras.length === 1 || camera === 'ptz' ? `<div class="media-speed-control"><div class="media-streams">${cameras.map(renderStreamControl).join('')}</div><output class="media-speed" aria-label="Current playback speed">${formatSpeed(DEFAULT_SPEED_FPS)}</output><input class="media-speed-slider" type="range" min="0" max="${SPEED_FPS.length - 1}" step="1" value="${DEFAULT_SPEED_INDEX}" aria-label="Playback speed" aria-valuetext="${html(formatSpeed(DEFAULT_SPEED_FPS))}"></div>${renderShortcutHelp()}` : '';
